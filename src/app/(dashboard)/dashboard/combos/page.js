...
function ModelItem({ id, index, model, isFirst, isLast, onEdit, onMoveUp, onMoveDown, onRemove }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex min-w-0 items-center gap-2 rounded-[6px] px-2.5 py-1.5 bg-surface border border-border hover:border-neutral-400 dark:hover:border-neutral-600 transition-colors select-none ${isDragging ? "shadow-md ring-1 ring-border" : ""}`}
    >
      {/* Drag handle */}
      <button
        {...attributes}
        {...listeners}
        type="button"
        className="cursor-grab touch-none p-1 rounded-[4px] text-text-muted hover:text-text-main active:cursor-grabbing shrink-0"
